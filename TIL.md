...
#### 7월 3일 리액트 심화과정
로그인 로그아웃 기능 구현 및 게시판 구성 및 게시글 목록 firebase 연동 하고 리덕스를 통해 상태관리하는 방법에 대해 관해 학습함. 리덕스 모듈에 액션 추가하고 리덕스 추가한 다음 스토어연결하고 그리고 디스패치를 통해 데이터를 주고받는과정은 반복된 작업이고 자료형이나 각 객체를 동일된 타입으로 통일하여 넣어주는 작업을 하는 것이 중요함. 결국 통일된 자료형의 객체로 데이터를 관리하고 넣어줘야되는 것.

#### 7월 5일 리액트 심화 강의 수강완료
리액트 강의 다 들었고 알림기능 및 댓글기능들은 직접 다시 한번 직접 구현해보고 깃에 업로드 해놓은 상태이다. 기본적으로 처음 데이터셋이 결정된 이후에 새로운 데이터들을 추가해서 보여주고 싶을때 원래 있던 데이터에 추가하기보다는 리덕스 모듈을 새로만들고 연동해서 쓰는 편이 더 깔끔한 듯 하다. 이제 좋아요 기능 추가해야되는데 새로운 리덕스 모듈을 한개 더 만들어서 추가해볼 예정이다.

#### 7월 7일 리액트 과제 진행중
알림 및 레이아웃 선택지 구성하는 것까지 완료하고 좋아요 기능 구현중에 redux랑 immer사용하는데 이해가 잘 안가고 어떤방식으로 쓰는지 감이 잘 안와서 그에 관한 공부가 필요할거 같다. 그리고 좋아요 기능을 구현할려면 먼저 로그인한 유저가 게시글에 좋아요를 눌렀는지 안눌렀는지를 판별하고 만약 누르지 않았다면 빈하트가 뜨게 하고 눌렀을 경우 빨간하트가 뜨게 하는건데 이걸 구현할려면 post객체안에 유저가 좋아요를 눌렀을 경우 아이디 정보를 담은 다음 로그인해 있는 유저의 정보와 db에 담긴 유저아이디의 정보가 같다면 빨간하트를 뜨게하고 아닐 경우 흰색 하트가 뜨게 하면 되는데.. 이걸 구현하는방법을 잘 모르겠어서 데이터 관리 및 상태관리에 대해 좀더 공부가 필요할거 같다.
#### 7월 8일 리액트 매거진사이트 완성
좋아요 기능을 구현할때 코드가 계속 난잡해졌던 이유가 한개의 함수안에 여러개의 기능을 넣어서 만들려다보니 계속 에러가 나고 작동이 안됬었던거 같다. 그래서 오늘 정답코드를 참고해서 좋아요 기능 구현을 하고 과제를 제출했다. 상태관리에 대해 아직 이해가 부족하고 어떻게 쓰는지 명확하게 알지 못해서 그런듯 앞으로도 프로젝트할때 모르는 부분은 세분화해서 각각의 함수들을 다 동작해보고 난 다음 그걸 프로젝트에 적용하는 방식으로 써야될듯 하다. 동시에 여러개를 할려니까 코드도 복잡해지고 점점 헷갈리기만 함
#### 7월 9~10일 미니프로젝트 시작
항해에서 처음으로 백엔드와 협업을 시작했다. 처음에 api설계도 그렇고 서로 알고 있는 기능이 서로 달라 처음에 살짝 당황스러웠다. 리액트같은 경우 리덕스를 이용해 페이지간 이동을 서버요청없이 기존 컴포넌트를 언마운트하고 새로운 컴포넌트를 업마운트해서 사용하기 때문에 처음부터 렌더링을 할 필요가 없다. 근데 백엔드 분들은 리액트를 처음 접하시는 분도 계서서 페이지간 이동의 개념이 달랐던 거 같다. 이것 외에도 굉장히 많은데 그래서 서로의 기능에 대해 어느정도라도 알고 있어야 의사소통이 좀더 원활하게 이뤄질거 같다.
